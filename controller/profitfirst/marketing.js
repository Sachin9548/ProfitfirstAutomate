...
const formattede = new Date(endDate).toISOString().split("T")[0];
  ...
  try {
    ...
    const params = {
      ...
      level: "campaign",
      fields: "campaign_id,spend,campaign_name,clicks,cpc,cpm,cpp,ctr,date_start,date_stop,frequency,inline_link_clicks,cost_per_unique_click,purchase_roas,website_purchase_roas,impressions,reach,action_values,actions"
    };
    ...
    try {
      ...
      console.error("Meta Ads Campaign Data Fetch Error:", err.message);
      return res.status(500).json({ error: "Failed to fetch campaign data from Meta Ads Manager" });
    }
    ...
    const campaignData = adsData.reduce((acc, campaign) => {
      ...
      (campaign.action_values || []).forEach(a => {
        if (a.action_type === 'purchase') totalSales += parseFloat(a.value) || 0;
      });
    ...
    const spendChartData = Object.entries(campaignData).map(([name, d]) => ({ name, spend: parseFloat(d.amountSpent), cpp: (parseFloat(d.costPerClick)*1000).toFixed(2), roas: parseFloat(d.roas) }));
    const adsChartData   = Object.entries(campaignData).map(([name, d]) => ({ name, value: parseFloat(d.roas) }));
    const analysisTable  = Object.entries(campaignData).map(([name, d]) => ({ name, spend: parseFloat(d.amountSpent), cpp: parseFloat(d.costPerClick), roas: parseFloat(d.roas) }));
    ...
    try {
      ...
      return res.status(500).json({ error: "Failed to fetch overall data from Meta Ads Manager" });
    }
    ...
    const totalSales = parseFloat((overallData.action_values||[]).find(a=>a.action_type==='purchase')?.value) || 0;
    const saleCount  = parseInt((overallData.actions||[]).find(a=>a.action_type==='purchase')?.value) || 0;
    ...
    const result = { summary, campaignMetrics: campaignData, spendChartData, adsChartData, analysisTable };
