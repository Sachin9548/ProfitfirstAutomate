...
// Helper component for individual input fields
const InputField = ({ label, value, onChange, placeholder, symbol = "₹" }) => (
  <div>
...
export default function ProfitCalculator() {
  ...
  useEffect(() => {
    ...
    const calculatedRevenue = numSellingPrice * numUnitsSold;
    const calculatedGrossProfit =
      (numSellingPrice - numCogsPerUnit) * numUnitsSold;

    const profitPerUnit =
      numSellingPrice - numCogsPerUnit - numShippingCostPerUnit;
    const totalUnitProfit = profitPerUnit * numUnitsSold;
    const calculatedNetProfit =
      totalUnitProfit - numMarketingSpend - numOtherExpenses;

    const calculatedNetMargin =
      calculatedRevenue > 0
        ? (calculatedNetProfit / calculatedRevenue) * 100
        : 0;

    const totalCosts =
      (numCogsPerUnit + numShippingCostPerUnit) * numUnitsSold +
      numOtherExpenses +
      numMarketingSpend;
  ...
  const COLORS = ["#33d17a", "#00d1ff", "#ff8c32", "#a56eff"]; // close to your screenshot
